...
class Tester(BaseTester):
    ...
    def test(self):
        ...
        with torch.no_grad():
            test_gts, test_res = [], []
            image_text_pairs = []  # 用来存储图像与生成文本的对应关系
            for batch_idx, (images_id, images, reports_ids, reports_masks) in tqdm(enumerate(self.test_dataloader)):
                ...
                # 保存每个图像与其对应的文本
                for img_id, report in zip(images_id, reports):
                    image_text_pairs.append([img_id, report])

            # 保存图像与生成的文本对应关系
            image_text_df = pd.DataFrame(image_text_pairs, columns=['Image_ID', 'Generated_Report'])
            image_text_df.to_csv(os.path.join(self.save_dir, "image_text_pairs.csv"), index=False, encoding='utf-8-sig')
            ...
            test_res.to_csv(os.path.join(self.save_dir, "res.csv"), index=False, header=False, encoding='utf-8-sig')
            test_gts.to_csv(os.path.join(self.save_dir, "gts.csv"), index=False, header=False, encoding='utf-8-sig')
